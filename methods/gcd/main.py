...
from config import distortions, severity, ovr_envs, clip_pretrain_path, dino_pretrain_path
...
if __name__ == "__main__":
    ...
    if args.dataset_name == 'domainnet':
        args.num_domains = 2 if args.task_type == 'A_L+A_U->B' else 3
    else:
        args.num_domains = 2 #len(distortions) + 1
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            with torch.no_grad():
                ...
                all_acc, old_acc, new_acc = test_kmeans(backbone, test_loader_A, epoch=epoch, save_name='Train ACC Unlabelled', args=args)
                ...
                if args.task_type == 'A_L+A_U+B->A_U+B+C':                                  
                    all_acc, old_acc, new_acc = test_kmeans(backbone, test_loader_B, epoch=epoch, save_name='Train ACC Unlabelled2', args=args)
                ...
                for test_loader in test_loaders: 
                    tmp_all_acc_test, tmp_old_acc_test, tmp_new_acc_test = test_kmeans(backbone, test_loader, epoch=epoch, save_name='Test ACC', args=args)
                ...
                # all_acc_test, old_acc_test, new_acc_test = test_kmeans(backbone, test_loader_C, epoch=epoch, save_name='Test ACC', args=args)

...
from config import fgvc_root, fgvcc_root, severity, distortions
...
class FGVCcAircraft(Dataset):
    ...
    def __init__(self, root, root_ori, base_folder, class_type='variant', split='train', transform=None,
                 target_transform=None, loader=default_loader, download=False):
        if split not in self.splits:
            raise ValueError('Split "{}" not found. Valid splits are: {}'.format(
                split, ', '.join(self.splits),
        ...
        self.class_type = class_type
        self.split = split
        self.classes_file = os.path.join(self.root_ori, 'data',
                                         'images_%s_%s.txt' % (self.class_type, self.split))
    ...
    def __getitem__(self, index):
        ...
        ori_path, target = self.samples[index]
        _, file_name = os.path.split(ori_path)

        path = os.path.join(self.root, self.base_folder, file_name)
    ...
    def download(self):
        ...
        # prepare to download data to PARENT_DIR/fgvc-aircraft-2013.tar.gz
        print('Downloading %s ... (may take a few minutes)' % self.url)
        parent_dir = os.path.abspath(os.path.join(self.root_ori, os.pardir))
        tar_name = self.url.rpartition('/')[-1]
        tar_path = os.path.join(parent_dir, tar_name)
        data = urllib.request.urlopen(self.url)
        ...
        # if necessary, rename data folder to self.root
        if not os.path.samefile(data_folder, self.root_ori):
            print('Renaming %s to %s ...' % (data_folder, self.root_ori))
            os.rename(data_folder, self.root_ori)
...
class MergedFGVCcAircraft(Dataset):
    class_types = ('variant', 'family', 'manufacturer')
    splits = ('train', 'val', 'trainval', 'test')

    def __init__(self, root, root_ori, args, class_type='variant', split='train', transform=None,
                 target_transform=None, loader=default_loader, download=False):
        if split not in self.splits:
            raise ValueError('Split "{}" not found. Valid splits are: {}'.format(
                split, ', '.join(self.splits),
    ...
    def __getitem__(self, index):
        ...
        ori_path, target = self.samples[index% len(self.samples)]
#         ori_path, target = self.samples[index-((int(sev)-1)*len(self.samples)*len(distortions)+(index//len(self.samples))*len(self.samples))]
    ...
    def __len__(self):
        return len(self.samples) * len(distortions) * len(severity)
...
def get_fgvcc_datasets(train_transform, test_transform, train_classes=range(50), prop_train_labels=0.8,
                    split_train_val=False, seed=0, args=None):

    np.random.seed(seed)

    if args.only_test:
        test_dataset = FGVCcAircraft(root=fgvcc_root, root_ori=fgvc_root, base_folder=args.distortion+'/'+args.severity, transform=test_transform, split='test')
        all_datasets = {
            'test': test_dataset,
        }
        return all_datasets
    ...
    if args.task_type == 'A_L+A_U->B':
        ...
        whole_training_set = FGVCAircraft(root=fgvc_root, transform=train_transform, split='trainval')
        ...
        # Init entire training set
        whole_training_set = FGVCAircraft(root=fgvc_root, transform=train_transform, split='test')
#                                           split='trainval')
        train_datasetB_unlabelled = MergedFGVCcAircraft(root=fgvcc_root, root_ori=fgvc_root, args=args, transform=train_transform, split='test')
        test_datasetB = MergedFGVCcAircraft(root=fgvcc_root, root_ori=fgvc_root, args=args, transform=test_transform, split='test')
...
if __name__ == '__main__':
    ...
    x = get_fgvcc_datasets(None, None, split_train_val=False)
